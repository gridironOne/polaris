...
if [[ $overwrite == "y" || $overwrite == "Y" ]]; then
	...
	./bin/grid init $MONIKER -o --chain-id $CHAINID --home "$HOMEDIR"
	...
	./bin/grid config set client chain-id "$CHAINID" --home "$HOMEDIR"

	# If keys exist they should be deleted
	for KEY in "${KEYS[@]}"; do
		./bin/grid keys add $KEY --keyring-backend $KEYRING --algo $KEYALGO --home "$HOMEDIR"
	done
	...
	for KEY in "${KEYS[@]}"; do
		./bin/grid genesis add-genesis-account $KEY 100000000000000000000000000afury --keyring-backend $KEYRING --home "$HOMEDIR"
	done
	...
	# Collect genesis tx
	./bin/grid genesis collect-gentxs --home "$HOMEDIR"

	# Run this to ensure everything worked and that the genesis file is setup correctly
	./bin/grid genesis validate-genesis --home "$HOMEDIR"
...
./bin/grid start --pruning=nothing "$TRACE" --log_level $LOGLEVEL --api.enabled-unsafe-cors --api.enable --api.swagger --minimum-gas-prices=0.0001afury --home "$HOMEDIR"