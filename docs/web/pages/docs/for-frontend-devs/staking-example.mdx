...
const Staking = () => {
    ...
    // State that holds the amount of tFury the connected account has delegated to the selected validator
    ...
    // Query the Staking Precompile for the amount of tFury delegated to the selected validator from the 
    ...
    // delegated_amount to the amount of tFury delegated to the selected validator.
    useMemo(() => {
        if (rawDelegation) {
    ...
    return (
        <Box
            ...
            <Flex justifyContent={'space-between'}>
                ...
                <Text>{delegated_amount ? delegated_amount : '0'} tFury</Text>
            </Flex>
            <Flex mt={4}>
                <Input
...
Congratulations! You have now created a staking application using the EVM! You can now delegate your tFury to a validator and earn rewards!
...
You can view this example and others like it on our examples repository on Github [here](https://github.com/gridironOne/polaris-examples)
</div>
